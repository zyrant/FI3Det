    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                #     num_sparse_gt = min(num, num_gt)
                #     annotations['gt_num'] = num_sparse_gt

                #     if annotations['gt_num'] != 0:

                #         choices = np.random.choice(num_gt, num_sparse_gt, replace=False)
                ...
                #             [obj.length, obj.width, obj.height] for obj in obj_list
                #             if obj.classname in self.cat2label.keys()
                #         ])[choices]  # lwh (depth) format
                #         annotations['rotation_y'] = np.array([
                #             obj.heading_angle for obj in obj_list
                #             if obj.classname in self.cat2label.keys()
                #         ])[choices]
                #         annotations['index'] = np.arange(
                #             len(obj_list), dtype=np.int32)[choices]
                #         annotations['class'] = np.array([
                #             self.cat2label[obj.classname] for obj in obj_list
                #             if obj.classname in self.cat2label.keys()
                #         ])[choices]
