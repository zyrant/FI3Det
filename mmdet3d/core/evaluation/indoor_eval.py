...
def indoor_eval(gt_annos,
    ...
    all_classes = list(label2cat.values())
    
    ret_dict = {}
    if len(NOVEL_CALSS) > 0:
        base_class = [c for c in all_classes if c not in NOVEL_CALSS]

        ret_dict_base = create_table(label2cat, ap, base_class, metric, rec, logger)
        ret_dict_new = create_table(label2cat, ap, NOVEL_CALSS, metric, rec, logger)
        ret_dict_all = create_table(label2cat, ap, all_classes, metric, rec, logger)
        
        ret_dict.update({'base_class': ret_dict_base})
        ret_dict.update({'new_class': ret_dict_new})
        ret_dict.update({'all_class': ret_dict_all})
    else:
        ret_dict_all = create_table(label2cat, ap, all_classes, metric, rec, logger)
    
        ret_dict.update(ret_dict_all)

    return ret_dict

def create_table(label2cat, ap, few_shot_class, metric, rec, logger):

    ret_dict = dict()
    header = ['classes']
    table_columns = [[label2cat[label]
                      for label in ap[0].keys() if label2cat[label] in few_shot_class] + ['Overall']]
    for i, iou_thresh in enumerate(metric):
        ...
        for label in ap[i].keys():
            if label2cat[label] not in few_shot_class:
                continue
            ret_dict[f'{label2cat[label]}_AP_{iou_thresh:.2f}'] = float(
                ap[i][label][0])
        ret_dict[f'mAP_{iou_thresh:.2f}'] = float(
            np.mean([ap[i][key] for key in ap[i].keys() if label2cat[key] in few_shot_class]))
        

        few_shot_values = [float(val[0]) for key, val in ap[i].items() if label2cat[key] in few_shot_class]

        table_columns.append(few_shot_values)
        table_columns[-1] += [ret_dict[f'mAP_{iou_thresh:.2f}']]
        table_columns[-1] = [f'{x:.4f}' for x in table_columns[-1]]

        # table_columns.append([ret_dict[f'mAP_{iou_thresh:.2f}']])
        # table_columns[-1] = [f'{x:.4f}' for x in table_columns[-1]]
        ...
        # table_columns.append([ret_dict[f'mAR_{iou_thresh:.2f}']])
        # table_columns[-1] += [f'{x:.4f}' for x in table_columns[-1]]
