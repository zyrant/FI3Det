...
if mode == 'random':
    inc_class_names = ('night_stand', 'chair', 'sofa', 'bookshelf', 'desk', 'dresser', 'bathtub', 'toilet', 'bed', 'table')
elif mode == 'count':
    inc_class_names = ('chair','table','desk','bed','sofa',
                              'night_stand','bookshelf','dresser','toilet','bathtub')
else:
    inc_class_names = None
current_stage = 2 # 1, 2
model = dict(
    type='Incremental_head',
    # change the pretrained model according to the current stage
    pretrained = '/opt/data/private/FI3Det/work-dir/sunrgbd/base_stage/5/epoch_12.pth',
    progressive_mode = True,
    total_stages=2,
    current_stage=current_stage,  # 1, 2
    stage_sizes = [3, 2],
...
dataset_type = 'SUNRGBDDataset_INC'
...
ann_root = '../all_data/sunrgbd/few_shot_increment/'
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            data_root=data_root,
            ann_file=ann_root + 'sunrgbd_infos_train_few_shot_incremental_' + mode +'.pkl',
